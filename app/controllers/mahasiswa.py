...
from app.forms.mahasiswa import MahasiswaSave, MataKuliahSave, LoginForm, LoginAdminForm
...
class AdminController:
    ...
    def login_adm(self):
        form = LoginAdminForm()
        if form.is_submitted():
            if form.nim.data == 'admin' and form.password.data == 'admin':
                return redirect(url_for('dash_admin'))
            else:
                return 'Password salah'
        return render_template('login/login_adm.html', form=form)
